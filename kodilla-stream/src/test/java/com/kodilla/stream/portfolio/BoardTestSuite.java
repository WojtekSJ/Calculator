...
class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        List<TaskList> inProgressTasks = new ArrayList<>();               // [1]
        inProgressTasks.add(new TaskList("In progress"));                 // [2]
        List longTasks = project.getTaskLists().stream()                  // [3]
                .filter(inProgressTasks::contains)                             // [4]
                .flatMap(tl -> tl.getTasks().stream())                         // [5]
                .map(Task::getCreated)                                         // [6]
                .map(d -> Duration.between(d.atStartOfDay(), LocalDate.now().atStartOfDay()).toDays())  // [7]
                .collect(toList());

        long start = 0;
        Long resultTasks = project.getTaskLists().stream()                  // [3]
                .filter(inProgressTasks::contains)                             // [4]
                .flatMap(tl -> tl.getTasks().stream())                         // [5]
                .map(Task::getCreated)                                         // [6]
                .map(d -> Duration.between(d.atStartOfDay(), LocalDate.now().atStartOfDay()).toDays())  // [7]
                .reduce(start, Long::sum);


        Double resultTasks2 = project.getTaskLists().stream()                  // [3]
                .filter(inProgressTasks::contains)                             // [4]
                .flatMap(tl -> tl.getTasks().stream())                         // [5]
                .map(Task::getCreated)                                         // [6]
                .map(d -> Duration.between(d.atStartOfDay(), LocalDate.now().atStartOfDay()).toDays())  // [7]
        ...
        assertEquals(difference, longTasks.get(1));
        assertEquals(difference2, longTasks.get(2));// [9]

        assertEquals(10.0, (double)(resultTasks/longTasks.size()));// [9]
        assertEquals(10.0, resultTasks2);
